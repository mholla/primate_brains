...
def shape_index(input_txt, subjects_name, output_folder, max_t, min_t):
    ...
    # Viridis Color Map
    plt.figure()
    ax = sns.kdeplot(t_cup_SI, color='#FDE725', shade=False, label='Cup',bw_adjust=2, cut=0.5)
    ax = sns.kdeplot(t_trough_SI, color='#B4DE2C', shade=False, label='Trough', bw_adjust=3,cut=0.5)
    ax = sns.kdeplot(t_rut_SI, color='#6DCD59', shade=False, label='Rut', bw_adjust=3,cut=0.5)
    ax = sns.kdeplot(t_srut_SI, color='#35B779', shade=False, label='Saddle_Rut', bw_adjust=3,cut=0.5)
    ax = sns.kdeplot(t_saddle_SI, color='#1F9E89', shade=False, label='Saddle', bw_adjust=3,cut=0.5)
    ax = sns.kdeplot(t_sridge_SI, color='#26828E', shade=False, label='Saddle_ridge', bw_adjust=4,cut=0.5)
    ax = sns.kdeplot(t_ridge_SI, color='#31688E', shade=False, label='Ridge', bw_adjust=3,cut=0.5)
    ax = sns.kdeplot(t_dome_SI, color='#3E4A89', shade=False, label='Dome', bw_adjust=4,cut=0.5)
    ax = sns.kdeplot(t_cap_SI, color='#482878', shade=False, label='Cap', bw_adjust=2,cut=0.5)
    ax.set(xlim=(0.1, 3))
    fname = os.path.join('/afs/crc.nd.edu/group/commandlab/Nagehan/curveball_scripts/plots', output_folder, 'si_curv_all.png')
    ...
    fname = os.path.join('/afs/crc.nd.edu/group/commandlab/Nagehan/curveball_scripts/plots', output_folder, 'si_curv_effect_all.png')
    plt.savefig(fname, dpi = 500)
    
    names = [('mean_cup', 'mean_trough', 'mean_rut', 'mean_srut', 'mean_saddle', 'mean_sridge', 'mean_ridge', 'mean_dome', 'mean_cap')]
    results = [(np.mean(t_cup_SI), np.mean(t_trough_SI), np.mean(t_rut_SI), np.mean(t_srut_SI), np.mean(t_saddle_SI), np.mean(t_sridge_SI), np.mean(t_ridge_SI), np.mean(t_dome_SI), np.mean(t_cap_SI))]
        
    SI_name = os.path.join('/afs/crc.nd.edu/group/commandlab/Nagehan/curveball_scripts/plots', output_folder, 'si_curv_all.asc')
    ...
    P_name = os.path.join('/afs/crc.nd.edu/group/commandlab/Nagehan/curveball_scripts/plots', output_folder, 'si_p_all.asc')
